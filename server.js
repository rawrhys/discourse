...
function buildRefinedSearchPhrases(subject, content, maxQueries = 10, courseTitle = '', options = {}) {
  ...
  for (const kw of baseKeywords) {
    ...
    // STRONGER FILTERING: Filter out generic terms that lead to irrelevant image results
    const genericTerms = [
      'early', 'period', 'dynasty', 'kingdom', 'empire', 'civilization', 'history', 'ancient', 'old', 'new', 
      'first', 'second', 'third', 'fourth', 'fifth', 'sixth', 'seventh', 'eighth', 'ninth', 'tenth', 
      'religion', 'society', 'formation', 'influence', 'development', 'growth', 'expansion', 'decline',
      'rise', 'fall', 'beginning', 'end', 'start', 'finish', 'creation', 'establishment', 'foundation',
      'origin', 'source', 'cause', 'effect', 'result', 'outcome', 'consequence', 'impact', 'significance',
      'importance', 'value', 'meaning', 'purpose', 'function', 'role', 'position', 'status', 'condition',
      'state', 'situation', 'circumstance', 'context', 'background', 'environment', 'setting', 'atmosphere'
    ];
      ...
      const modulePromises = courseWithIds.modules.map(async (module, mIdx) => {
        ...
        const lessonPromises = module.lessons.map(async (lesson, lIdx) => {
          ...
          try {
            ...
            let lessonContentString = null;
            let retryCount = 0;
            const maxRetries = 3;
            
            while (!lessonContentString && retryCount < maxRetries) {
              try {
                console.log(`[AIService] Generating lesson "${lesson.title}" (attempt ${retryCount + 1}/${maxRetries})`);
                lessonContentString = await this._makeApiRequest(lessonPrompt, 'lesson', false);
                if (lessonContentString) {
                  console.log(`[AIService] Successfully generated lesson "${lesson.title}" on attempt ${retryCount + 1}`);
                  break;
                }
              } catch (retryError) {
                retryCount++;
                console.warn(`[AIService] Lesson generation attempt ${retryCount} failed for "${lesson.title}":`, retryError.message);
            ...
            if (!lessonContentString) {
              throw new Error(`Failed to generate lesson content after ${maxRetries} attempts`);
            }
...
Return only the JSON array, no other text.`;
            ...
            lesson.content = {
              introduction: `Welcome to the lesson on ${lesson.title}. This lesson will cover key concepts and provide comprehensive understanding of the topic.`,
              main_content: `The content for ${lesson.title} is currently being generated by our AI service. This lesson will include detailed explanations, examples, and key terms to help you master the subject matter. Please refresh the page or try generating the course again to get the full lesson content.`,
              conclusion: `Once the AI service completes generation, you'll have access to a complete lesson with detailed explanations, examples, and key takeaways for ${lesson.title}.`
            };
            lesson.quiz = [];
            lesson.flashcards = [];
          }
...
async function initializeDatabase() {
  try {
    ...
    // Migrate existing users to have required email verification fields
    await migrateExistingUsers();
    
    console.log('[DB] Database initialized successfully at:', dbFilePath);
    console.log('[DB] Users in database:', db.data.users.map(u => ({ id: u.id, email: u.email, courseCredits: u.courseCredits, emailVerified: u.emailVerified })));
  } catch (error) {
...
async function migrateExistingUsers() {
  try {
    ...
    for (const user of db.data.users) {
      ...
      // Check if user needs email verification fields
      if (user.emailVerified === undefined) {
        user.emailVerified = true; // Mark existing users as verified
        needsMigration = true;
      }
      
      // Check if user needs verification token fields
      if (user.verificationToken === undefined) {
        user.verificationToken = null;
        needsMigration = true;
      }
      
      if (user.verificationTokenCreatedAt === undefined) {
        user.verificationTokenCreatedAt = null;
        needsMigration = true;
      }
      
      // Check if user needs emailVerifiedAt field
      if (user.emailVerifiedAt === undefined) {
        user.emailVerifiedAt = user.emailVerified ? new Date().toISOString() : null;
        needsMigration = true;
      }
      
      // Check if user needs password field for local users
      if (user.password === undefined && user.id.startsWith('local_')) {
        user.password = null; // Will need to be set on next login
        needsMigration = true;
      }
      
      if (needsMigration) {
        migrationCount++;
        console.log(`[DB_MIGRATION] Migrated user: ${user.email} (${user.id})`);
      }
    }
    
    if (migrationCount > 0) {
      await db.write();
      console.log(`[DB_MIGRATION] Successfully migrated ${migrationCount} users`);
    } else {
      console.log('[DB_MIGRATION] No users needed migration');
    }
  } catch (error) {
    console.error('[DB_ERROR] Failed to migrate existing users:', error);
    // Don't throw error to avoid breaking server startup
  }
}
...
app.get('/api/auth/verify-email', async (req, res) => {
  try {
    ...
    console.log('[EMAIL VERIFY] GET: Attempting to verify email with token:', token.substring(0, 10) + '...', 'for email:', email);

    // Find user with this verification token
    const user = db.data.users.find(u => u.verificationToken === token && !u.emailVerified);
    if (!user) {
      console.log('[EMAIL VERIFY] GET: No user found with token or email already verified');
    ...
    if (tokenAge > tokenExpiry) {
      console.log('[EMAIL VERIFY] GET: Token expired for user:', user.email);
    ...
    console.log('[EMAIL VERIFY] GET: Email verified successfully for:', user.email);
    return res.json({ 
      success: true, 
...
// Email verification endpoint - POST method for backward compatibility
...
app.post('/api/auth/login', async (req, res) => {
  ...
  try {
    ...
    console.log('[LOGIN] Using local authentication (no captcha required)');
...
app.get('/api/auth/me', authenticateToken, (req, res) => {
  ...
  console.log(`[API] /api/auth/me called for user ${user.id}:`, {
    id: user.id,
    email: user.email,
    name: user.name,
    courseCredits: user.courseCredits,
    hasCredits: user.courseCredits !== undefined && user.courseCredits !== null,
...
// SSE endpoint removed - no longer needed
  ...
  async (req, res) => {
    try {
      ...
      if (firstName.trim().length < 2 || lastName.trim().length < 2) {
        return res.status(400).json({ error: 'firstName and lastName must be at least 2 characters long' });
      }
      ...
      if (!session) {
        ...
        try {
          ...
          console.log(`[API] Created new session ${actualSessionId} for course ${courseId}`);
        } catch (createError) {
      ...
      if (!session) {
        console.error(`[API] Failed to get or create session for ${sessionId}`);
        return res.status(500).json({ error: 'Failed to get or create session' });
      }
      
      // Update the session with the username - use direct property assignment
      try {
        ...
        console.error(`[API] Error setting username:`, error);
        res.status(500).json({ error: 'Failed to set username' });
      }
    } catch (error) {
...
// Update student progress (lesson completion, quiz scores, etc.)
app.post('/api/public/courses/:courseId/student-progress', 
  securityHeaders,
  securityLogging,
  botDetection,
  publicCourseRateLimit,
  ...
  async (req, res) => {
    try {
      const { courseId } = req.params;
      const { sessionId, lessonId, moduleId, quizScore, action } = req.body;
      
      if (!sessionId) {
        return res.status(400).json({ error: 'Missing sessionId' });
      }
      
      console.log(`[API] Updating student progress for course:`, {
        courseId,
        sessionId,
        lessonId,
        moduleId,
      ...
      const updatedProgress = studentProgressService.getStudentProgress(sessionId);
      ...
      console.error('[API] Failed to update student progress:', error);
...
app.get('/api/billing/status', authenticateToken, async (req, res) => {
  try {
    ...
    const subscriptions = (subs?.data || []).map(s => ({
      ...
      items: (s.items?.data || []).map(it => ({ price: it.price?.id, product: it.price?.product, nickname: it.price?.nickname, unit_amount: it.price?.unit_amount, currency: it.price?.currency }))
    }));
...
app.get('/api/billing/check-subscription-status', authenticateToken, async (req, res) => {
  try {
    ...
    // First check local database status as it might be more up-to-date
    const dbUser = db.data.users.find(u => u.id === userId);
    if (dbUser?.subscriptionStatus === 'canceled') {
      console.log(`[BILLING] User ${userId} has canceled subscription in local DB`);
      return res.json({
    ...
    if (!stripe) {
      ...
      const hasActiveLocal = dbUser?.subscriptionStatus && 
        ['active', 'trialing', 'past_due', 'unpaid'].includes(dbUser.subscriptionStatus);
      
      return res.json({ 
        hasActiveSubscription: hasActiveLocal || false, 
        status: dbUser?.subscriptionStatus || null,
        message: hasActiveLocal ? 'Active subscription found in local DB' : 'No active subscription found'
      });
    }
    ...
    if (activeSubscription) {
      ...
      if (dbUser && dbUser.subscriptionStatus !== activeSubscription.status) {
        dbUser.subscriptionStatus = activeSubscription.status;
        dbUser.updatedAt = new Date().toISOString();
        await db.write();
        console.log(`[BILLING] Updated local DB subscription status to: ${activeSubscription.status}`);
      }
      ...
      console.log(`[BILLING] User ${userId} has no active subscriptions in Stripe`);
    ...
    // Fall back to local database check on error
    try {
      ...
      return res.json({ 
        hasActiveSubscription: hasActiveLocal || false, 
        status: dbUser?.subscriptionStatus || null,
        message: hasActiveLocal ? 'Active subscription found in local DB (fallback)' : 'No active subscription found (fallback)'
      });
    } catch (fallbackError) {
...
app.post('/api/billing/refresh-subscription-status', authenticateToken, async (req, res) => {
  try {
    const userId = req.user.id;
    console.log(`[BILLING] Manual refresh requested for user ${userId}`);
    
    // Force a fresh check from Stripe
    const status = await checkSubscriptionStatusFromStripe(userId);
    
    // Update local database
    const dbUser = db.data.users.find(u => u.id === userId);
    if (dbUser) {
      dbUser.subscriptionStatus = status.hasActiveSubscription ? status.status : 'canceled';
      if (!status.hasActiveSubscription) {
        dbUser.subscriptionCanceledAt = new Date().toISOString();
      }
      dbUser.updatedAt = new Date().toISOString();
      await db.write();
      console.log(`[BILLING] Updated local DB for user ${userId}: ${dbUser.subscriptionStatus}`);
    }
    
    res.json(status);
  } catch (error) {
    console.error('[BILLING] Error refreshing subscription status:', error);
    res.status(500).json({ 
...
// Helper function to check subscription status from Stripe
async function checkSubscriptionStatusFromStripe(userId) {
  if (!stripe) {
    return { hasActiveSubscription: false, message: 'Stripe not configured' };
  }
  ...
  if (!customerId) {
    return { hasActiveSubscription: false, message: 'No Stripe customer found' };
  }
  ...
  try {
    ...
    console.error('[BILLING] Stripe API error:', error);
    throw error;
  }
}
