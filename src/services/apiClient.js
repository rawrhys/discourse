...
const apiClient = async (url, options = {}) => {
  ...
  try {
    ...
    if (!response.ok) {
      ...
      const errorText = await response.text();
      const errorData = RobustJsonParser.parse(errorText, 'API Error Response') || {};
    ...
    if (url.includes('/api/courses/generate')) {
      ...
      if (!contentType || !contentType.includes('text/event-stream')) {
        ...
        const responseData = RobustJsonParser.parse(responseText, 'Non-Streaming Fallback');
        if (responseData) {
      ...
      try {
        while (true) {
          ...
          for (const line of lines) {
            if (line.trim() && line.startsWith('data: ')) {
              ...
              const data = RobustJsonParser.parse(line.slice(6), 'Streaming Chunk');
              if (data) {
        ...
        if (buffer.trim() && buffer.startsWith('data: ')) {
          ...
          const data = RobustJsonParser.parse(buffer.slice(6), 'Final Streaming Chunk');
          if (data) {
